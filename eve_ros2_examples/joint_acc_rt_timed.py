...
class WholeBodyCommandPublisher(Node):
    ...
    def __init__(self, whole_body_command_msg=None):
        ...
        # Keep time, must be a native ROS function for this instead
        self.t = 0.0
        self.dt = 0.002  
    ...
    def timer_callback(self):
        ...
        # Depending on the time, set another reference acceleration            
        if self.t >= self.on_time:
            self._whole_body_command_msg.joint_space_commands.append(generate_joint_space_acc_command_msg(self.joint, 0.0))
            self.get_logger().info("Done, destroy timer")
        ...
        # Send message and increment current time
        self._publisher.publish(self._whole_body_command_msg)
        self.t += self.dt

    # Set torque to zero before exiting on early ctrl+c
    def reset(self):
        # Create message
        self._whole_body_command_msg = WholeBodyControllerCommand();
        self._whole_body_command_msg.joint_space_commands.append(generate_joint_space_acc_command_msg(self.joint, 0.0))
